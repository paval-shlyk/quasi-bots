...
impl FinanceMcpServer {
    ...
    #[tool(
        description = "List all currencies supported by DZENGI.com exchange"
    )]
    ...
    #[tool(
        description = "Get the order book (bids/asks) for a symbol, e.g. BTC/USD"
    )]
    async fn get_order_book(
        ...
        match exchange::get_order_book(&self.api, &p.symbol).await {
            Ok(ob) => serde_json::to_string_pretty(&ob)
                .unwrap_or_else(|e| format!("error: {e}")),
            Err(e) => format!("error: {e}"),
        }
    }
    ...
    #[tool(
        description = "Get exchange metadata: all trading pairs, base/quote assets, status"
    )]
    ...
    #[tool(
        description = "Get OHLCV candlestick data for a symbol. interval examples: 1m, 5m, 15m, 1h, 4h, 1d"
    )]
    async fn get_klines(
        &self,
        Parameters(p): Parameters<KlinesParams>,
    ) -> String {
        match exchange::get_klines(&self.api, &p.symbol, &p.interval).await {
            Ok(klines) => serde_json::to_string_pretty(&klines)
                .unwrap_or_else(|e| format!("error: {e}")),
            Err(e) => format!("error: {e}"),
        }
    }
    ...
    #[tool(
        description = "Get account information: balances, permissions, commission rates"
    )]
    async fn get_account_balances(&self) -> String {
        match account::get_account(&self.api).await {
            Ok(acc) => serde_json::to_string_pretty(&acc)
                .unwrap_or_else(|e| format!("error: {e}")),
            Err(e) => format!("error: {e}"),
        }
    }
    ...
    async fn fetch_order(
        &self,
        Parameters(p): Parameters<OrderIdParams>,
    ) -> String {
    ...
    #[tool(
        description = "Get open orders. Leave symbol empty to fetch all open orders"
    )]
    async fn get_open_orders(
        &self,
        Parameters(p): Parameters<OptionalSymbolParams>,
    ) -> String {
        let sym = if p.symbol.is_empty() {
            None
        } else {
            Some(p.symbol.as_str())
        };
    ...
    #[tool(
        description = "Get the full ledger (all entries) for a currency. Leave currency empty for all currencies"
    )]
    async fn get_ledger(
        &self,
        Parameters(p): Parameters<CurrencyParams>,
    ) -> String {
        let cur = if p.currency.is_empty() {
            None
        } else {
            Some(p.currency.as_str())
        };
    ...
    #[tool(
        description = "Get all deposit and withdrawal transactions (full history, paginated automatically)"
    )]
    ...
    #[tool(
        description = "Get closed trading position history. Leave symbol empty for all symbols"
    )]
    ...
    #[tool(
        description = "Get 24-hour price statistics (OHLC, volume, change%) for a symbol"
    )]
    async fn get_ticker(
        &self,
        Parameters(p): Parameters<SymbolParams>,
